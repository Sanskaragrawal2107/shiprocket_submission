...
export const AdsDashboardSchema = z.object({
  campaigns: z.array(z.object({
    ...
    spend:       z.number().describe("Amount spent in INR"),
  ...
  return (
    <div className="data-card" style={{ minWidth: 340, maxWidth: 620 }}>
      ...
      <div className="data-card-body">
        {(total_spend !== undefined || avg_roas !== undefined) && (
            ...
            <tbody>
              {campaigns.slice(0, 6).map((c, i) => (
                <tr key={i}>
                  ...
                  <td>{c.ctr !== undefined ? `${c.ctr.toFixed(1)}%` : "—"}</td>
                  <td style={{
                    fontWeight: 600,
                    color: (c.roas ?? 0) >= 2 ? "var(--accent-emerald)" : "var(--accent-rose)"
                  }}>
                    {c.roas !== undefined ? `${c.roas.toFixed(1)}x` : "—"}
                  </td>
                </tr>
              ))}
            </tbody>
